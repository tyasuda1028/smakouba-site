...
- **独自ドメイン**: `sumakouba.com`（apex＝ルートドメイン）
...
### 6. リポジトリに独自ドメインを設定
`smakouba-site` の **Settings → Pages → Custom domain** に `sumakouba.com` を入力して **Save**。
→ mainブランチ直下に `CNAME` ファイル（中身 `sumakouba.com`）が自動コミットされる。DNSが既に解決済みなので、現行 `tyasuda1028.github.io/smakouba-site/` も問題なく新ドメインへ転送される。

### 7. HTTPSを有効化
DNS検証が通ると GitHub が Let's Encrypt証明書を自動発行（利用可能まで最大24時間）。発行後、Settings → Pages の **Enforce HTTPS** にチェック。

### 8. 最終確認
- ブラウザで `https://sumakouba.com/` が表示される。
- `https://www.sumakouba.com/` が `sumakouba.com` へリダイレクトされる。
...
- apexを主にする弱点：将来 GitHub のIPが変わると追従が必要（過去数年は安定）。より安定を取るなら `www.sumakouba.com` を主（Custom domainに `www.sumakouba.com` を設定）にし、apexをリダイレクト元にする構成も可。
